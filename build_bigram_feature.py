...
def main(argv):
   ...
   if (vocabf):
      ...
      outfile= codecs.open(path+"/"+outputf+"_bigram_vocab_"+str(word_count_threshold)+".txt", 'w',"utf-8-sig")
      for t in vocab:
        outfile.write(t[0] + "\t" + t[1] + "\n")
      outfile.close()
   #write to binary file for large data set
   bow.tofile(path+"/"+outputf+"_bag_of_bigrams_"+str(word_count_threshold)+".dat")
